...
\item December 5th, 2025 - Added the settling time to the list of
  parameter for the time constant assignment and edited the overall
  assignment directions to have a better description of the video requirements. Also edited the servo second order systems equation for an aircraft to be stable since the equations were unstable open loop. 
\end{enumerate}

\section*{Changes Needed}
\begin{enumerate}[itemsep=-5pt]
\item Move all of these changes over to Github issues for better
  tracking
\item For assignments with figures have the students annotate each
  figure and include supporting text
\item Also make all the equations they have to simulate somewhat
  custom made like the second order pitch response of an aircraft
\item Make them create wiring diagrams for every assignment
...
\item Add annotations describing components of the kit in the main kit
  picture
\item For the kit purchase assignment make sure you put in bold text
  something to the effect of ``don't buy anything until you talk with
  your instructor"
\end{enumerate}

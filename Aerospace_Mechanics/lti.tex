...
For a satellite undergoing free 1-D rotational motion the dynamics are quite interesting from a control perspective given the somewhat marginal stability of the satellite. In this case, it is assumed that the spacecraft rotates through the angle $\theta$ about its center of mass. The spacecraft has a moment of inertia $J$ and is subject to an external torque $\tau$ as shown in Figure \ref{f:satellite}. In this case ,the torque is created by reaction control thrusters with a force $F$ at a distance $d$ from the center of mass. Note that there are 4 thrusters but only 2 operate at a time. If the top right and bottom left thrusters fire the torque is negative and if the top left and bottom right thrusters fire the torque is positive.
\begin{figure}[H]
    \centering
    \begin{tikzpicture}[scale=1.2]
        ...
        \draw[->,thick] (0.25,1.8) -- ++(0,-0.5) node[midway, right] {$F$};
        \draw[->,thick] (1.75,1.8) -- ++(0,-0.5) node[midway, right] {$F$};
        \draw[->,thick] (0.25,-0.8) -- ++(0,0.5) node[midway, right] {$F$};
        \draw[->,thick] (1.75,-0.8) -- ++(0,0.5) node[midway, right] {$F$};
...
\end{equation}
where $\vec{\alpha}_{B/I}$ is the angular acceleration of the satellite. The torque generated by the thrusters is given as $\vec{M}_C = \tau = 2Fd$. The multiplication by 2 is due to 2 thrusters firing at a time. The final equations of motion for the satellite are then given as
\begin{equation}
...
\begin{equation}
    q(t) = \frac{f_0}{2} t^2
\end{equation}
In the case of the satellite example, recall that $f_0 = \frac{2F_0 d}{J}$. This result is intuitive because it states that if a constant force to the thrusters is applied, the angle of the satellite will increase quadratically over time. That is, the system will continue to accelerate until it runs out of fuel or the thrusters are turned off.
